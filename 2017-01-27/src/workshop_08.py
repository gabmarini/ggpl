...
def texturized_floors(houseType, story, ladderHoleModel = False):
	...
	"""
	res = []
	def build_floor(houseType, roomType, texturePrefix):
		counter = 1
		result = []
		while True:
			if os.path.isfile("lines/house"+ houseType + "/" + roomType + str(story) + str(counter) + ".lines"):
				with open("lines/house"+ houseType + "/" + roomType + str(story) + str(counter) + ".lines", "rb") as file:
					reader = csv.reader(file, delimiter=",")
					polylineList = []
					for row in reader:
						polylineList.append(POLYLINE([[float(row[0]), float(row[1])],[float(row[2]), float(row[3])]]))
				...
				break
		return result
	res = res + build_floor(str(houseType), "bedroom","camera")
	res = res + build_floor(str(houseType), "bathroom", "bagno")
	res = res + build_floor(str(houseType), "livingroom", "sala")
	res = res + build_floor(str(houseType), "terrace_surface", "terrazzo")
	return res


def build_house(type, story, windowsGenerationFunction = False, doorsGenerationFunction = False, ladderHoleModel = False):
	"""
	...
	externalWalls = generate_2D_walls("house" + str(type) + "/" + "muriesterni"+str(story))
	...
	internalWalls = generate_2D_walls("house" + str(type) + "/" + "muriinterni"+str(story))
	...
	if(not doorsGenerationFunction):
		doors = generate_hole_models("house" + str(type) + "/" + "porte"+str(story))
		doors = PROD([doors, Q(2.5/xfactor)])
	else:
		doors = generate_doors_special_hole_models("house" + str(type) + "/" + "porte_model"+str(story), doorsGenerationFunction, 3/zfactor)

	#building 3D-windows holes
	if(not windowsGenerationFunction):
		windows = generate_hole_models("house" + str(type) + "/" + "finestre"+str(story))
		windows = PROD([windows, Q(SIZE([3])(walls)[0]/2.)])
		windows = T(3)(SIZE([3])(walls)[0]/4.)(windows)
	else:
		windows = generate_windows_special_hole_models("house" + str(type) + "/" + "finestre_model"+str(story),windowsGenerationFunction,4/zfactor)

	#building 2D-model of the terrace walls
	terrace_walls = generate_2D_walls("house" + str(type) + "/" + "terrace_walls"+str(story))
	...
	exteriors = TEXTURE(["texture/ext" + str(randint(1,10)) + ".jpg",True,True,10,10,-PI/2.,1,1])(exteriors)
	interiors = TEXTURE(["texture/wood1.jpg",True,True,1,1,PI/2.,5,5])(interiors)

	#building the floors
	floor = STRUCT(texturized_floors(type, story, ladderHoleModel))
